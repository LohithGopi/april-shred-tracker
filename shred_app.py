...
def process_video_ai(video_path, exercise_name):
    """
    Scans video using MediaPipe to check form.
    Returns: Path to processed video, Feedback Dictionary
    """
    ...
    # Generate Feedback
    feedback = {"status": "success", "msg": "Analysis Complete", "details": []}
    ex_name = exercise_name.lower()
    
    if "squat" in ex_name or "leg" in ex_name:
        if min_knee_angle < 90:
            feedback["details"].append("✅ Excellent Depth (< 90°)")
        elif min_knee_angle < 110:
            feedback["details"].append("⚠️ Good depth, but try to go lower.")
        else:
            feedback["status"] = "warning"
            feedback["msg"] = "Depth Issue"
            feedback["details"].append("❌ You are not breaking parallel.")
            
    elif "press" in ex_name or "push" in ex_name:
        if min_elbow_angle < 75:
            feedback["details"].append("✅ Good Range of Motion")
        else:
            feedback["details"].append("⚠️ Lower the weight to get a deeper stretch.")
            
    else:
        feedback["details"].append(f"Min Knee Angle: {int(min_knee_angle)}°")
        feedback["details"].append(f"Min Elbow Angle: {int(min_elbow_angle)}°")
        
    return output_temp, feedback
...
with col_main:
    ...
    # B. WORKOUT (WITH AI VIDEO CHECKER)
    st.markdown('<div class="glass-card">', unsafe_allow_html=True)
    ...
    if exercise_list:
        for ex in exercise_list:
            with st.expander(f"💪 {ex['ex']}"):
                # CREATING TABS: Guide vs AI Check
                tab1, tab2 = st.tabs(["📝 Guide & Log", "🤖 AI Form Check"])
                
                # TAB 1: Normal Logging
                with tab1:
                ...
                with tab2:
                    st.info(f"Upload a video of you doing {ex['ex']} to get AI feedback.")
                    uploaded_file = st.file_uploader(f"Upload Video for {ex['ex']}", type=['mp4', 'mov'], key=f"vid_{ex['ex']}")
                    ...
                    if uploaded_file is not None:
                        ...
                        if st.button(f"Analyze Form ⚡", key=f"btn_{ex['ex']}"):
                            with st.spinner("🤖 AI Vision is scanning keypoints (Knees, Back, Elbows)..."):
                                try:
                                    processed_path, feedback = process_video_ai(tfile.name, ex['ex'])
